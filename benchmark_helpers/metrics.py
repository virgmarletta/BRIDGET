...
def compute_deferral_metrics(data_test):
    ...
    """
    results = {}
    print(f"Unique values in defers: {np.unique(data_test['defers'])}")
    # returna dict metric:accuracy score
...
def compute_classification_metrics(data_test, method=None, ds_name= None):
    """compute metrics for just classification

    Args:
        data_test (dict): dict data with fields 'labels',  'preds'
        method (str, optional): method name
        ds_name (str, optional): dataset name, for saving purposes
    ...
    if method is not None and  ds_name is not None:
        base_dir = DATASETS[ds_name]['baseline_paths']['benchmark_res']
        folder = os.path.join(base_dir, method
        )
        os.makedirs(folder, exist_ok=True)
        folder_path= os.path.join(folder, f"classification_metrics.parquet")

        results=pd.DataFrame([results])
        results.to_parquet(folder_path, index=False)

    return results


def compute_coverage_v_acc_curve(data_test, method=None, ds_name= None):
    """
    ...
    if method is not None and  ds_name is not None:
        base_dir = DATASETS[ds_name]['baseline_paths']['benchmark_res']
        folder = os.path.join(base_dir, method,
                              f"coverage_v_acc_curve")
        os.makedirs(folder, exist_ok=True)

        save_path = os.path.join(folder, "all_metrics.parquet")
